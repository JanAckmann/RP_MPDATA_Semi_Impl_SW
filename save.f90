...
INTEGER, PARAMETER :: ML_INPUT=65, ML_BIAS=0
!INTEGER, PARAMETER :: ML_INPUT=15
INTEGER, PARAMETER :: ML_INPUT1=9
...
double precision :: Layer1(M,ML_INPUT+ML_BIAS,1) , &
               &     Layer2(M,6  ,1), &
               &     max_val,         &
               &     help_layer(ML_INPUT+ML_BIAS), &
               &     help_layer1(ML_INPUT1+ML_BIAS)
INTEGER          :: Order_Layer1(M,ML_INPUT), max_ind, k


!! read in weights for L0N0, 5x5 preconditioner
Layer1(:,ML_INPUT+ML_BIAS,1)=0.0d0
do i=0,29
  help_layer(:)=0.0d0
  write(latitude_layer,*) i
   filename_NN= &
&'weights_Cross/FortranWeights_model30x2_1x0_l0w1_METRICnorm_onlyR_noBias_uselinear_noinputact'&
& //trim(adjustl(latitude_layer))//'_layer0.txt'

  call read_layer(filename_NN,help_Layer(:),(ML_INPUT),1)
  k=1
  do j=1,ML_INPUT,1
  !do j=1,ML_INPUT+ML_BIAS,1!7
    Layer1(i+1,j,1)=help_layer(k)
    k=k+1
  enddo
! write(*,*) i+1, Layer1(i+1,1:ML_INPUT+ML_BIAS,1)
! read(*,*)
enddo
...
do i=30,M-1-30
  ...
  write(latitude_layer,*) i
   filename_NN= &
&'weights_Cross/FortranWeights_model2x2_1x0_l0w1_METRICnorm_onlyR_noBias_uselinear_noinputact'&
& //trim(adjustl(latitude_layer))//'_layer0.txt'

  call read_layer(filename_NN,help_Layer1(:),(ML_INPUT1),1)
  !k=1
  !do j=1,ML_INPUT,1
  !do j=1,ML_INPUT+ML_BIAS,1!7
    Layer1(i+1,29:29+4,1)=help_layer1(1:5)
    Layer1(i+1,62:65,1)=help_layer1(6:9)
    !k=k+1
  !enddo
 !write(*,*)  Layer1(i+1,1:ML_INPUT+ML_BIAS,1)
 !read(*,*)
enddo
Order_Layer1(:,:)=0
...
!do i=0,M-1
!  write(latitude_layer,*) i
!  filename_NN= 'weights/FortranWeights_model2x2_1x0_l0w1coeff_R_uselinear_noinputact'//trim(adjustl(latitude_layer))//'_layer0.txt' 
!  call read_layer(filename_NN,Layer1(i+1,:,:),177,1)
!  !write(*,*) i+1, Layer1(i+1,1:177,1)
!  !read(*,*)
!enddo
!
!Order_Layer1(:,:)=0
...
do ID_PREC=7,7,-5
 do IRHW = 3,3
  ...
  !ID_PREC=0
   EXP_NAME= 'data/data_ADI_NOTgcr_EXP3_Dp_1M10_MLPrecon_iterINF_FULL_res4'
...
! finished writing fields for ML
  ...
  &      niter,nitsm,icount,error, PD_T, sum_time, sum_lp_time, ID_PREC,.FALSE., save_time,&
       ...
       &  EXP_NAME, iprint, num_of_bits, DP_Depth,kt, Alp_REL, Layer1, layer2, ML_INPUT, ML_BIAS)
...
Open(unit=599, file=trim(path)//trim(file_name), status='replace', form = 'unformatted', access='direct', recl=8)

zaehl=1
do I=2,N-1
  do J=1,M
    write(unit=599, rec=zaehl) H(I,J)
    zaehl=zaehl+1
  end do
end do
...
subroutine GCR_PRE(p,pfx,pfy,hx,hy,s,S_full,b,p0,pb,e1,e2,cor,ip  &
           ...
           & , iprint, num_of_bits, DP_Depth,kt, Alp_REL, Layer1, Layer2, ML_I, ML_B)
use implicit_functions_DP


implicit none
INTEGER, parameter :: kord=4, lord=kord-1 !kord=4 is default

INTEGER :: n1, n2, ML_I, ML_B
...
!ML preconditioner
double precision :: Layer1(n2 ,ML_I+ML_B,1), &
                 &  Layer2(n2 ,6,1) 
...
double precision :: inflation_factor0, inflation_factorit, ML_beta
integer :: upper_L, lower_L
upper_L =5
lower_L =5
...
lowprectime=0.0d0
 ...
 ! call write_MLfields_iter(p(:,:), KT, n1, n2, EXP_NAME, 'H_iter', 0)
...
!    do J=0,n2-1
      ...
      NN_r_HP(:,:)= (r_HP(2:n1-1,:))/9.80616
      NNa11(:,:) = (a11(2:n1-1,:)-8832816614973.127 )/(370735007156489.9 - 285797600451.5306)
      NNa12(:,:) = a12(2:n1-1,:)/(2.0d0*40927556954.599144 )
      NNa21(:,:) = (a21(2:n1-1,:)-1632540745224.2573 )/(2814357893215.7007 - 5861279991.116582) 
      NNa22(:,:) = a22(2:n1-1,:)/(2.0d0*40927556954.59915)
      NNb11(:,:) =  b11(2:n1-1,:)/(2.0d0*12286263193393.56)
      NNb22(:,:) = b22(2:n1-1,:)/(2.0d0*163170483547.3604 )
!    enddo
call ML_precon(MLx(:,:),Layer1, Layer2,NN_Y,NN_r_HP,NNa11,&
          & NNa12,NNa21,NNa22,NNb11,NNb22,n1-2,n2,ML_I, ML_B)


  x(:,:,1)=MLx(:,:)*9.80616

! call write_MLfields_iter(x(:,:,1), KT, n1, n2, EXP_NAME, 'ML_x', 0)
! endif
...
!call write_MLfields(b22(:,:), KT, n1, n2, EXP_NAME, 'b22')
  ...
  !! this sum could be problematic
    ...
    DO J=20,n2
      DO I=1,n1
        rax=rax+r(I,J)*ax(I,J,1)
        ax2(1)=ax2(1)+ax(I,J,1)*ax(I,J,1)
      enddo
    enddo
...
call cpu_time(startLP)
!else
call precon(r,MLx(:,:),ax(:,:,1), T_step,  A_c, ps, divi,a11,a12,a21,a22,b11,b22,p0,  &
                &   pfx,pfy,s,S_full,n1,n2,ip,ID_PREC, num_of_bits, DP_Depth)
!x(:,1:27,1)=MLx(:,1:27)
!x(:,227:256,1)=MLx(:,227:256)

x(:,1:lower_L,1)=MLx(:,1:lower_L)
x(:,256-upper_L:256,1)=MLx(:,256-upper_L:256)
  
call adjust_conservation(x(:,:,1),s,S_full,n1,n2)
!endif        
   ! do j=1,n2
   !    do i=1,n1
   !     write(*,*)  'qu',i, J, x(i,j,1)
   !    enddo
   !    read(*,*)
   !   enddo
        !endif
...
do it=1,itr
 ...
 If (it==1000) then
  ...
  do l=1,lord
    ...
    write(*,*) 'beta', beta, 'ML_beta', ML_beta
...
!  call write_MLfields_iter(p(:,:), KT, n1, n2, EXP_NAME, 'H_iter', niter+1)
!  call write_MLfields_iter(r_HP(:,:), KT, n1, n2, EXP_NAME, 'R_iter',niter+1)
...
!  call write_MLfields_iter(qu(:,:), KT, n1, n2, EXP_NAME, 'ML_x', niter+1)
! endif
!else
call precon(r,MLx, aqu , T_step,  A_c, ps, divi,a11,a12,a21,a22,b11,b22,p0,   &
                &   pfx,pfy,s,S_full,n1,n2,ip,ID_PREC, num_of_bits, DP_Depth)
!endif
qu(:,1:lower_L)=MLx(:,1:lower_L)
qu(:,256-upper_L:256)=MLx(:,256-upper_L:256)

  call adjust_conservation(qu(:,:),s,S_full,n1,n2)
!endif
!    do j=1,n2
!       do i=1,n1
!        write(*,*)  'qu',i, J, qu(i,j)
...
subroutine ML_precon (QU,Layer1, Layer2,NN_Y,NNr_HP,NN_a11,NN_a12,NN_a21,NN_a22,NN_b11,NN_b22,&
                &  N, M, ML_I, ML_B)
use implicit_functions_DP
implicit none
integer :: ML_I, ML_B
double precision :: QU(N+2,M)
double precision :: Layer1(M ,ML_I+ML_B,1), &
                 &  Layer2(M ,6,1) 
double precision :: NNr_HP(N,M),NN_a11(N,M),NN_a12(N,M),NN_a21(N,M),NN_a22(N,M),NN_b11(N,M),NN_b22(N,M), NN_Y(M)
INTEGER :: N, M

double precision :: ML_input0(ML_I),ML_input1(5), scaling
...
Do J=0,M-1
  ...
  do I=0,N-1
   ...
   ! get stencil from fields NN_Y,r_HP,NN_a11,NN_a12,NN_a21,NN_a22,NN_b11,NN_b22
     ...
     ML_input0(1:ML_I)=ML_input0(1:ML_I) /scaling
   !if(j==0) then
   !write(*,*) J, I, counter, ML_I, ML_B
   !write(*,*) (ML_input0(k), k=1,counter-1)
   !write(*,*) 'Weights'
   !write(*,*) (Layer1(J+1,k,1), k=1,counter-1)

   !read(*,*)
   !endif
   ! apply neural network
   !call apply_layer_ReLu(ML_input0,Layer1(J+1,:,:),ML_input1,177,5)
   !write(*,*) 'apply layer lin begin'
   call apply_layer_Lin(ML_input0,Layer1(J+1,:,:),QU((I+1)+1,J+1),ML_I+ML_B,1)
   !write(*,*) 'apply layer lin end'
   QU((I+1)+1,J+1)=QU((I+1)+1,J+1)*scaling
   !if(j==0) then
   !write(*,*) 'ML_Output'
   !write(*,*) QU(I+1+1,J+1),scaling 
   !read(*,*)
   !endif
...
double precision :: Input(In_dim-1)
double precision :: Weights(In_dim, 5)
...
integer :: In_dim, Out_dim
double precision :: Input(In_dim-1)
double precision :: Weights(In_dim,1)
double precision :: Output(1)
double precision :: summand
integer :: I, J


!write(*,*) 'apply layer lin'
do J=1,Out_dim
