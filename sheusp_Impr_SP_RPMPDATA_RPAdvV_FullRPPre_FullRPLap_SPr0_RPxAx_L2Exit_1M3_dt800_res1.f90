...
do ID_PREC=7,7,-5
 do DP_Depth_nr=3,3,1
  ...
  !ID_PREC=0
EXP_NAME= 'data/sheusp_IMPR_SP_RPMP_RPAdv_FRPPreAll_FRPLap_SPr0V2_RPxAx_DP'//trim(adjustl(Dp_depth_str))//'_L2Exit_1M3_dt800_res1'
...
IF(IANAL.EQ.0) THEN
 ...
 !   CALL VELPRD(U,V,F1,F2,PD,HX,HY,IP,N,M,GC1,GC2,EP, KMX)
    CALL VELPRD_RP(U,V,F1,F2,PD,HX,HY,IP,N,M,GC1,GC2,EP, KMX)
 !   endif
...
! call add_OldForces_dp(QX_HP, QY_HP,F1_dp(:,:,0), F2_dp(:,:,0) , N, M)
 ...
 IF (codesignQ) then
    ...
    !  CALL MPDATT(UA,VA,QY,S,N,M,IORD,ISOR,NONOS,IDIV,-1, IP,liner, QY_T, codesignQ)
      ...
      CALL MPDATT_RP(UA,VA,QX,S,N,M,IORD,ISOR,NONOS,IDIV,-1, IP,liner, QX_T, codesignQ, DP_Depth)
      CALL MPDATT_RP(UA,VA,QY,S,N,M,IORD,ISOR,NONOS,IDIV,-1, IP,liner, QY_T, codesignQ, DP_Depth)
...
SUBROUTINE MPDATT_RP(U1_SP,U2_SP,X_SP,H_SP,N,M,IORDs,ISOR,NONOS,IDIV,IBC, IP,liner, X_T, codes, DP_Depth_MPDATA)
...
implicit none
  ...
  REAL(kind=4) :: U1_SP(N,M),U2_SP(N,M+1),X_SP(N,M),H_SP(N,M), X_T(N,M)
  type(rpe_var) :: U1(N,M),U2(N,M+1),X(N,M),H(N,M)
  INTEGER :: IP(N)
  INTEGER  :: IORD, IORDs, ISOR, NONOS, IDIV, IBC , DP_Depth_MPDATA
      

  type(rpe_var) :: V1(N,M),V2(N,M+1),F1(N,M),F2(N,M+1)  &
      &      ,CP(N,M),CN(N,M)   
  type(rpe_var) :: MX(N,M),MN(N,M)
  type(rpe_var) :: EP, C1, C2, V1D, V2D, V2D1, V2DN
  INTEGER :: N1, N2, I, J, K, liner
  LOGICAL :: codes
  type(rpe_var) :: V1_RP(N,M), &
                 & V2_RP(N,M+1), &
                 & X_RP(N,M),& 
                 & H_RP(N,M), & 
                 & MX_RP(N,M), &
  ...
  ! COMPUTE DONOR-CELL FLUXES
   ...
   ! COMPUTE NEW UPWIND-SOLUTION
    ...
    IF(.not. (K.EQ.IORD)) then  ! GO TO 6
      ...
      MX_RP(:,:)=MX(:,:)
      MN_RP(:,:)=MN(:,:)
    call MPDATT_RP_2ndOrd(V1_RP,V2_RP,X_RP,H_RP,MX_RP,MN_RP,N1,N2,MP,LW,IORD,ISOR,NONOS,IDIV,IBC, IP, codes, DP_Depth_MPDATA)
...
implicit none
  ...
  type(rpe_var) :: X(N1,N2),H(N1,N2)
  INTEGER :: IP(N1)
  INTEGER  :: IORD, ISOR, NONOS, IDIV, IBC , DP_Depth_MPDATA
      

  type(rpe_var) :: V1(N1,N2),V2(N1,N2+1),F1(N1,N2),F2(N1,N2+1)  &
      &      ,CP(N1,N2),CN(N1,N2)   
  type(rpe_var) :: MX(N1,N2),MN(N1,N2)
  type(rpe_var) :: EP, C1, C2, V1D, V2D, V2D1, V2DN
  INTEGER ::  I, J, MP,LW
  LOGICAL :: codes
...
SUBROUTINE VELPRD_RP(U,V,F,G,PD,HX,HY,IP,N,M,A,B,EP,KMX)
use implicit_functions_RPE
use rp_emulator
...
REAL(Kind=4) ::  U(N,M,0:1),V(N,M,0:1),F(N,M,0:1),G(N,M,0:1),         &
     &          PD(N,M),HX(N,M),HY(N,M)
type(rpe_var) ::  U_RP(N,M,0:1),V_RP(N,M,0:1),F_RP(N,M,0:1),G_RP(N,M,0:1),         &
...
REAL(Kind=4) ::  CF, C1, C2, C1H, C2H, ALFA, BETA, ALF1, BET1, ALFM, BETM
REAL(Kind=4) :: AMP, UF(N),VF(N),SCR(N)
type(rpe_var)  :: AMP_RP, UF_RP(N),VF_RP(N),SCR_RP(N), EP_RP
INTEGER :: IORT, I, J
...
DO J=1,M
  DO I=1,N
    F_RP(I,J,1)=U(I,J,0)+CF*F(I,J,0)/max(PD(I,J),EP)
    G_RP(I,J,1)=V(I,J,0)+CF*G(I,J,0)/max(PD(I,J),EP)
  enddo
end do
...
implicit none
type(rpe_var) :: U(N,M), V(N,M), F(N,M), G(N,M), HX(N,M), HY(N,M), UU(N,M), VV(N,M), C1, C2
integer :: IP(M), N, M, J, I
type(rpe_var) :: ALFA, BETA, ALF1, BET1, ALFM, BETM
