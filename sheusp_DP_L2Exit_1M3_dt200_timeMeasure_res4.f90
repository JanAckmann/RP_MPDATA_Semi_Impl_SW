...
! the rest
DOUBLE PRECISION :: GMM, SUM1, SUM0, start, finish, GCR_start, GCR_finish, GCR_sum,&
       &  MPDATA_start, MPDATA_finish, MPDATA_sum &
       & , advVel_start, advVel_finish, advVel_sum&
       & , donor_start, donor_finish, donor_sum&
       & , PseudoVel_start, PseudoVel_finish,PseudoVel_sum
...
call cpu_time(advVel_finish)
advVel_sum=advVel_sum+(advVel_finish-advVel_start)
   ! adv_cour=0.0d0
   !
   ! DO J=1,M
   !   DO I=1,N
...
call cpu_time(MPDATA_start)
      ! predict fluid thickness for a second order accurate implicit problem
      CALL MPDATT(UA,VA,PC,S,N,M,IORD,ISOR,NONOS,IDIV,1, IP,liner, PD_T, .FALSE.,donor_sum, PseudoVel_sum)

      CALL MPDATT(UA,VA,QX,S,N,M,IORD,ISOR,NONOS,IDIV,-1, IP,liner, QX_T, codesignQ,donor_sum, PseudoVel_sum)
      CALL MPDATT(UA,VA,QY,S,N,M,IORD,ISOR,NONOS,IDIV,-1, IP,liner, QY_T, codesignQ,donor_sum, PseudoVel_sum)
      
call cpu_time(MPDATA_finish)
MPDATA_sum=MPDATA_sum+(MPDATA_finish-MPDATA_start)
...
call cpu_time(GCR_start)
 ...
 If(save_time) exit  ! if iteration counter has reached 100 once, jump to next set of bits
   ...
   call cpu_time(GCR_finish)
   GCR_sum=GCR_sum+(GCR_finish-GCR_start)
...
write(*,*) 'Total runtime,advVel, MPDATA, GCR', finish-start,advVel_sum, MPDATA_sum, GCR_sum
write(*,*)' DonorCell scheme, PseudeVel', donor_sum, PseudoVel_sum
END program
...
SUBROUTINE MPDATT(U1,U2,X,H,N,M,IORDs,ISOR,NONOS,IDIV,IBC, IP,liner, X_T, codes, donor_sum, pseudoVel_sum)
...
implicit none
  ...
  DOUBLE PRECISION :: V1(N,M),V2(N,M+1),F1(N,M),F2(N,M+1)  &
      &      ,CP(N,M),CN(N,M), donor_sum, pseudoVel_sum
  DOUBLE PRECISION :: donor_start, pseudoVel_start, donor_finish, pseudoVel_finish
  DOUBLE PRECISION :: MX(N,M),MN(N,M)
  DOUBLE PRECISION :: EP, C1, C2, V1D, V2D, V2D1, V2DN
  INTEGER :: N1, N2, I, J, K, liner
  LOGICAL :: codes

  !call cpu_time(donor_start)
  ...
  call cpu_time(pseudoVel_finish)

  pseudoVel_sum=pseudoVel_sum+(pseudoVel_finish-pseudoVel_start) 
  
  C1=rpe_1
  C2=rpe_0
  
  !call cpu_time(donor_finish)
  !donor_sum=donor_sum+(donor_finish-donor_start)
